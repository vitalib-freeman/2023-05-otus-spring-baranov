...
class RunnerTest {
    ...
    private void mockMessageSource(List<Question> questions) {
        when(messageProvider.getTranslation(eq("strings.hello"))).thenReturn("Hi, pls enter your name:");
        when(messageProvider.getTranslation(eq("strings.resultPass"))).thenReturn("Congratulations, you have passed the test");
